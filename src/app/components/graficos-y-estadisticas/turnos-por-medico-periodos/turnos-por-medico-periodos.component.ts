...
@Component({
  selector: 'turnos-por-medico-periodos',
  standalone: true,
  imports: [
    MatFormFieldModule, MatDatepickerModule, ReactiveFormsModule, FormsModule, CommonModule
  ],
  providers: [provideNativeDateAdapter()],
  templateUrl: './turnos-por-medico-periodos.component.html',
  styleUrl: './turnos-por-medico-periodos.component.scss'
})
export class TurnosPorMedicoPeriodosComponent implements OnInit, OnChanges {
  ...
  titulo:string = 'Cantidad de Turnos por Médicos en un lapso';
  ...
  resetFiltro(){
    ...
    this.turnosFiltradosPorDia = this.turnos;
  ...
  crearBarChartTurnosPorMedico(){
    ...
    const data = {
      ...
      datasets: [{
        label: 'Cantidad de turnos solicitado por médico',
        data: this.especialistas.map(item => turnosPorMedico[item.uid] || 0), 
        backgroundColor: this.backgroundColor
      }]
    };

    this.chartTurnosPorMedico = new Chart('barChartTurnosPorMedicoPeriodo', {
      type: 'bar',
      data: data,
      options:{
        responsive: true,
